'use client';
import { useEffect, useState } from 'react';
import AuthGuard from '@/components/AuthGuard';
import CmsImageUpload from '@/components/CmsImageUpload';
import { getCampaigns, createCampaign, updateCampaign, deleteCampaign, toggleCampaign } from '@/lib/cms-firestore';
import { Campaign } from '@/types';
import { Plus, Edit2, Trash2, Eye, EyeOff, X, Check, Megaphone } from 'lucide-react';

const EMPTY: Omit<Campaign, 'id' | 'createdAt'> = {
  title: '', description: '', imageUrl: '', startDate: '', endDate: '',
  isActive: true, targetType: 'mixed',
  linkedExperienceIds: [], linkedEventIds: [], linkedEstablishmentIds: [],
};

function CampaignsContent() {
  const [campaigns, setCampaigns] = useState<Campaign[]>([]);
  const [loading,   setLoading]   = useState(true);
  const [saving,    setSaving]    = useState(false);
  const [toast,     setToast]     = useState('');
  const [editing,   setEditing]   = useState<(Omit<Campaign, 'id' | 'createdAt'> & { id?: string }) | null>(null);
  const [confirm,   setConfirm]   = useState<string | null>(null);

  function showToast(msg: string) { setToast(msg); setTimeout(() => setToast(''), 3000); }
  async function load() { setCampaigns(await getCampaigns()); }
  useEffect(() => { load().finally(() => setLoading(false)); }, []);

  async function save() {
    if (!editing?.title) { showToast('Titre requis.'); return; }
    setSaving(true);
    try {
      if (editing.id) {
        const { id, ...data } = editing as Campaign;
        await updateCampaign(id, data);
        showToast('Campagne mise à jour ✓');
      } else {
        await createCampaign(editing as Omit<Campaign, 'id' | 'createdAt'>);
        showToast('Campagne créée ✓');
      }
      setEditing(null); load();
    } catch { showToast('Erreur.'); }
    finally { setSaving(false); }
  }

  async function handleDelete(id: string) {
    await deleteCampaign(id);
    setConfirm(null); showToast('Campagne supprimée'); load();
  }

  if (loading) return <div className="flex justify-center mt-20"><div className="w-10 h-10 border-4 border-solar border-t-transparent rounded-full animate-spin" /></div>;

  return (
    <div>
      {toast && <div className="fixed top-20 right-4 z-50 bg-anthracite text-white px-5 py-3 rounded-2xl shadow-soft font-semibold animate-fadeUp">{toast}</div>}

      <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
        <div>
          <h1 className="font-display font-bold text-4xl text-anthracite flex items-center gap-3">
            <Megaphone size={30} className="text-solar" /> Campagnes
          </h1>
          <p className="text-gray-500 mt-1">{campaigns.filter(c => c.isActive).length} active{campaigns.filter(c => c.isActive).length > 1 ? 's' : ''} · {campaigns.length} total</p>
        </div>
        <button onClick={() => setEditing({ ...EMPTY })}
          className="bg-solar text-white px-5 py-3 rounded-2xl font-bold text-sm flex items-center gap-2 hover:bg-orange-600 transition">
          <Plus size={16} /> Nouvelle campagne
        </button>
      </div>

      {/* Form */}
      {editing && (
        <div className="bg-white rounded-4xl shadow-soft p-6 mb-8 border-2 border-solar/20">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-display font-bold text-2xl">{editing.id ? 'Modifier' : 'Nouvelle'} campagne</h2>
            <button onClick={() => setEditing(null)} className="text-gray-400 hover:text-gray-600"><X size={22} /></button>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">Titre *</label>
                <input type="text" value={editing.title}
                  onChange={e => setEditing(p => ({ ...p!, title: e.target.value }))}
                  placeholder="Ex: Été à Abidjan 2025"
                  className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-solar/30 focus:border-solar" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">Description</label>
                <textarea rows={3} value={editing.description}
                  onChange={e => setEditing(p => ({ ...p!, description: e.target.value }))}
                  placeholder="Décris l'objectif de cette campagne…"
                  className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-solar/30 resize-none" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">Type de cible</label>
                <select value={editing.targetType}
                  onChange={e => setEditing(p => ({ ...p!, targetType: e.target.value as Campaign['targetType'] }))}
                  className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-solar/30">
                  <option value="experience">Expériences</option>
                  <option value="event">Événements</option>
                  <option value="establishment">Établissements</option>
                  <option value="mixed">Mixte</option>
                </select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-gray-500 mb-1">Date début</label>
                  <input type="date" value={editing.startDate}
                    onChange={e => setEditing(p => ({ ...p!, startDate: e.target.value }))}
                    className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-solar/30" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 mb-1">Date fin</label>
                  <input type="date" value={editing.endDate}
                    onChange={e => setEditing(p => ({ ...p!, endDate: e.target.value }))}
                    className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-solar/30" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">IDs expériences liées (virgule)</label>
                <input type="text" value={editing.linkedExperienceIds.join(', ')}
                  onChange={e => setEditing(p => ({ ...p!, linkedExperienceIds: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }))}
                  placeholder="id1, id2…"
                  className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-solar/30" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">IDs événements liés (virgule)</label>
                <input type="text" value={editing.linkedEventIds.join(', ')}
                  onChange={e => setEditing(p => ({ ...p!, linkedEventIds: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }))}
                  placeholder="id1, id2…"
                  className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-solar/30" />
              </div>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={editing.isActive}
                  onChange={e => setEditing(p => ({ ...p!, isActive: e.target.checked }))}
                  className="rounded accent-orange-500 w-4 h-4" />
                <span className="text-sm font-medium">Campagne active</span>
              </label>
            </div>
            <div>
              <CmsImageUpload
                folder="cms/campaigns"
                value={editing.imageUrl}
                onChange={url => setEditing(p => ({ ...p!, imageUrl: url }))}
                label="Image de la campagne"
                height={260}
              />
            </div>
          </div>
          <div className="mt-5 flex gap-3">
            <button onClick={save} disabled={saving}
              className="bg-solar text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-orange-600 transition disabled:opacity-60">
              {saving ? <span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Check size={18} />}
              {editing.id ? 'Mettre à jour' : 'Créer'}
            </button>
            <button onClick={() => setEditing(null)} className="border border-gray-200 px-6 py-3 rounded-2xl font-bold text-gray-600 hover:bg-gray-50 transition">
              Annuler
            </button>
          </div>
        </div>
      )}

      {campaigns.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-4xl shadow-card">
          <p className="text-5xl mb-4">📣</p>
          <h3 className="font-display font-bold text-xl">Aucune campagne</h3>
          <p className="text-gray-500 mt-2">Crée ta première campagne promotionnelle.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {campaigns.map(c => (
            <div key={c.id} className="bg-white rounded-3xl shadow-card overflow-hidden">
              <div className="flex items-start gap-4 p-5">
                {c.imageUrl && (
                  <img src={c.imageUrl} alt={c.title} className="w-28 h-20 rounded-xl object-cover shrink-0" />
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div>
                      <p className="font-display font-bold text-lg">{c.title}</p>
                      {c.description && <p className="text-sm text-gray-500 mt-0.5 line-clamp-2">{c.description}</p>}
                      <p className="text-xs text-gray-400 mt-1">
                        {c.startDate} → {c.endDate} · {c.targetType}
                      </p>
                    </div>
                    <span className={`text-xs font-bold px-3 py-1.5 rounded-full shrink-0 ${c.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'}`}>
                      {c.isActive ? '✅ Active' : '⏸️ Inactive'}
                    </span>
                  </div>
                </div>
              </div>
              <div className="px-5 pb-4 flex gap-2 border-t border-gray-50 pt-3">
                <button onClick={() => setEditing(c)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 text-xs font-medium hover:bg-gray-50 transition">
                  <Edit2 size={13} /> Modifier
                </button>
                <button onClick={() => toggleCampaign(c.id, !c.isActive).then(load)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 text-xs font-medium hover:bg-gray-50 transition">
                  {c.isActive ? <><EyeOff size={13} /> Désactiver</> : <><Eye size={13} /> Activer</>}
                </button>
                {confirm === c.id ? (
                  <div className="flex gap-1.5 items-center">
                    <span className="text-xs text-gray-500">Confirmer ?</span>
                    <button onClick={() => handleDelete(c.id)} className="px-2.5 py-1.5 bg-red-600 text-white rounded-xl text-xs font-bold">Oui</button>
                    <button onClick={() => setConfirm(null)} className="px-2.5 py-1.5 bg-gray-100 rounded-xl text-xs">Non</button>
                  </div>
                ) : (
                  <button onClick={() => setConfirm(c.id)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 text-xs font-medium text-red-500 hover:bg-red-50 hover:border-red-200 transition">
                    <Trash2 size={13} /> Supprimer
                  </button>
                )}
              </div>
            </div>
          ))}
        </div>
      )}
    </div>
  );
}

export default function AdminCampaignsPage() {
  return (
    <main className="max-w-5xl mx-auto px-4 py-10">
      <AuthGuard adminOnly><CampaignsContent /></AuthGuard>
    </main>
  );
}
