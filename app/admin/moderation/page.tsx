...
function ModerationContent() {
  ...
  return <div>
    ...
    {actionTarget && <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 px-4"><div className="w-full max-w-md rounded-3xl bg-white p-6"><h2 className="font-display text-xl font-bold">{actionTarget.status === 'approved' ? 'Approuver' : 'Rejeter'} « {actionTarget.name} »</h2><p className="mt-2 text-sm text-gray-500">{actionTarget.status === 'rejected' ? 'Le motif sera visible par l’annonceur.' : 'Une remarque reste facultative.'}</p><textarea rows={4} value={reason} onChange={e => setReason(e.target.value)} className="mt-4 w-full rounded-xl border border-gray-200 p-3 outline-none focus:border-solar" placeholder="Motif ou remarque"/><div className="mt-4 flex gap-2"><button onClick={confirmAction} className="rounded-xl bg-solar px-5 py-2.5 font-bold text-white">Confirmer</button><button onClick={() => setActionTarget(null)} className="rounded-xl bg-gray-100 px-5 py-2.5 font-bold text-gray-600">Annuler</button></div></div></div>}
