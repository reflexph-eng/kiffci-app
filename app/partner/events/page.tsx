...
import { getMyEvents, deleteEvent } from '@/lib/partner-firestore';
import { KiffEvent } from '@/types';
import Link from 'next/link';
import { Plus, Edit2, Trash2, Eye, Phone, MessageCircle, Calendar } from 'lucide-react';
...
function EventsContent() {
  ...
  async function load() {
    if (!appUser) return;
    setEvents(await getMyEvents(appUser.uid));
  }
          ...
          {events.map(e => (
            <div key={e.id} className="bg-white rounded-3xl shadow-card p-5">
              <div className="flex items-start gap-4">
                {e.images[0] && (
                  <img src={e.images[0]} alt={e.title} className="w-20 h-20 rounded-2xl object-cover shrink-0" />
                )}
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 flex items-center gap-1.5 mt-0.5">
                        ...
                        {' → '}
                        {new Date(e.endDate).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}
                      </p>
                      <p className="text-sm text-gray-500">📍 {e.location}, {e.city}</p>
                    </div>
              ...
              <div className="mt-4 flex gap-2 flex-wrap">
                <Link href={`/partner/edit-event/${e.id}`}
                ...
                {confirm === e.id ? (
                  <div className="flex gap-2 items-center">
                    ...
                    <button onClick={() => handleDelete(e.id)} className="px-3 py-1.5 bg-red-600 text-white rounded-xl text-sm font-bold">Oui</button>
                    <button onClick={() => setConfirm(null)} className="px-3 py-1.5 bg-gray-100 rounded-xl text-sm">Non</button>
                  </div>
                ) : (
