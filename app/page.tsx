...
export default function Home() {
  ...
  useEffect(() => {
    Promise.all([getExperiences(), getApprovedEstablishments()])
      .then(([experienceItems, establishmentItems]) => {
  ...
  const featuredEsts =
    settings.featuredEstablishmentIds.length > 0
      ? ests
          .filter((e) => settings.featuredEstablishmentIds.includes(e.id))
          .slice(0, 14)
      : ests.slice(0, 14);
    ...
    return (
      <main className="flex min-h-screen items-center justify-center px-4">
        <div className="max-w-md text-center">
          ...
          <h1 className="mb-4 font-display text-3xl font-bold text-anthracite">
            Maintenance en cours
          </h1>
          <p className="text-gray-500">
            Kiffci est en cours de mise à jour. Reviens dans quelques minutes.
          </p>
        </div>
      </main>
    );
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-center gap-x-8 gap-y-2">
            ...
            <button
              ...
              className="inline-flex min-h-11 items-center gap-2 border-b border-anthracite/35 pb-1 text-sm font-semibold text-anthracite/80 transition hover:border-solar hover:text-solar"
            >
      ...
      <section className="site-container page-section pb-8 pt-4 lg:pb-12 lg:pt-5">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <h2 className="section-heading font-display font-bold">
              Expériences à vivre
            </h2>
          </div>
          <Link
            href="/experiences"
            className="flex shrink-0 items-center gap-1 text-sm font-semibold text-solar transition-all hover:gap-2"
          >
        ...
        {loading ? (
          <div
            ...
            aria-label="Chargement des expériences"
          >
          ...
          <div className="py-10 text-center text-gray-400">
            <p>
              Aucune expérience.{" "}
              <Link href="/admin" className="text-solar hover:underline">
                Injecte les données démo →
              </Link>
            </p>
          </div>
        )}
      ...
      {featuredEsts.length > 0 && (
        <section className="site-container py-8 lg:py-12">
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              <p className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-solar">
                Celles et ceux qui font vivre la Côte d’Ivoire
              </p>
              <h2 className="section-heading font-display font-bold">
                Créateurs d’expériences à découvrir
              </h2>
            </div>
            <Link
              href="/establishments"
          ...
          >
            {featuredEsts.map((est) => (
              ...
              >
                ...
                <div className="pt-3">
                  <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-solar">
                    {est.category}
                  </p>
                  <h3 className="mt-1 line-clamp-2 font-display text-sm font-bold leading-snug text-anthracite transition-colors group-hover:text-solar md:text-[15px]">
                    {est.ownerName || est.name}
                  </h3>
            ...
            <section className="site-container pb-8">
              <div className="relative h-52 overflow-hidden bg-tropical">
                ...
                <div className="relative flex h-full items-center p-8">
                  <div>
                    <span className="mb-3 inline-block bg-white/20 px-3 py-1 text-xs font-bold text-white">
        ...
        <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/50 px-4 pb-4 animate-fadeUp sm:items-center">
          <div className="relative w-full max-w-sm bg-white p-6">
            <button
              onClick={() => setRec(null)}
              aria-label="Fermer"
              className="absolute right-4 top-4 z-10 text-gray-300 transition hover:text-gray-500"
            >
              <X size={18} />
            </button>
            <h2 className="mb-4 pr-8 font-display text-xl font-bold">
            ...
            <div className="mt-4 flex gap-3">
              <button
                onClick={handleSurprise}
                className="flex-1 bg-gray-100 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-200"
              >
                <Shuffle size={14} className="mr-1.5 inline" />
                Une autre
              </button>
              <button
                onClick={() => setRec(null)}
                className="flex-1 bg-anthracite py-2.5 text-sm font-medium text-white transition hover:opacity-90"
              >
                Fermer
              </button>
            </div>
        ...
        <div className="relative overflow-hidden bg-anthracite p-10 text-white md:p-16">
          ...
          <div className="relative max-w-2xl">
            ...
            <p className="mb-8 text-gray-300">
              Valide chaque expérience, monte de niveau et débloque des badges
              exclusifs Kiffci.
            </p>
            <Link
              href="/passport"
              className="inline-flex items-center gap-2 bg-solar px-6 py-3.5 font-bold text-white transition hover:bg-orange-600"
            >
