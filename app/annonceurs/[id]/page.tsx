...
export default function PublicPartnerPage(){
  ...
  const name=experiences[0]?.ownerName||establishments[0]?.ownerName||events[0]?.organizerName||'Annonceur KIFFCI';
  ...
  return <main className="pb-20">
    <section className="bg-anthracite text-white"><div className="site-container py-10 md:py-16"><Link href="/experiences" className="inline-flex items-center gap-2 text-sm text-white/60 hover:text-white"><ArrowLeft size={16}/>Retour aux expériences</Link><div className="mt-10 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[.22em] text-solar">Vitrine annonceur</p><div className="mt-3 flex flex-wrap items-center gap-3"><h1 className="font-display text-4xl font-bold md:text-6xl">{name}</h1>{verified&&<span className="inline-flex items-center gap-1 rounded-full bg-white/10 px-3 py-1 text-sm font-bold"><BadgeCheck size={16} className="text-tropical"/>Vérifié</span>}</div><p className="mt-5 max-w-2xl text-white/70">Découvre les expériences proposées, les lieux qui les accueillent et les rendez-vous datés de cet annonceur.</p>{cities.length>0&&<p className="mt-4 inline-flex items-center gap-2 text-sm text-white/60"><MapPin size={15}/>{cities.slice(0,4).join(' · ')}</p>}</div><div className="grid grid-cols-3 gap-px overflow-hidden rounded-2xl bg-white/10"><Stat icon={Sparkles} value={experiences.length} label="expériences"/><Stat icon={Building2} value={establishments.length} label="lieux"/><Stat icon={CalendarDays} value={events.length} label="datées"/></div></div></div></section>
