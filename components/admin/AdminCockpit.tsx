...
export default function AdminCockpit(props: Props) {
 return <section className="mb-10 space-y-8">
  ...
  <div className="grid gap-5 lg:grid-cols-[1.35fr_.65fr]">
   ...
   <div className="grid gap-x-8 gap-y-6 md:grid-cols-2">{groups.map(group=><div key={group.title}><p className="mb-2 text-[11px] font-bold uppercase tracking-[0.16em] text-gray-400">{group.title}</p><div className="divide-y divide-gray-100 border-t border-gray-100">{group.items.map(({href,icon:Icon,label,sub})=><Link key={`${group.title}-${label}`} href={href} className="group flex items-center gap-3 py-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-50 text-gray-500 transition group-hover:bg-solar/10 group-hover:text-solar"><Icon size={15}/></span><span className="min-w-0 flex-1"><span className="block text-sm font-semibold text-anthracite">{label}</span><span className="block truncate text-xs text-gray-400">{sub}</span></span><ArrowRight size={14} className="text-gray-300 transition group-hover:translate-x-0.5 group-hover:text-solar"/></Link>)}</div></div>)}</div></div>
   <aside className="rounded-2xl border border-gray-100 bg-white p-5"><p className="text-xs font-bold uppercase tracking-[0.16em] text-gray-400">Outils système</p><h2 className="mt-1 font-display text-2xl font-bold text-anthracite">Maintenance</h2><p className="mt-2 text-sm leading-6 text-gray-500">Actions techniques isolées du parcours quotidien.</p><div className="mt-5 space-y-2">
