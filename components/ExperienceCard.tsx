...
export default function ExperienceCard({ e, badge, featured = false, compact = false }: Props) {
  const resolvedBadge = badge ?? getEditorialBadgeFromHighlight(e) ?? computeAutoBadge(e.createdAt, e.isPremium || e.isSponsored);

  return (
    <article className={`group relative border-b border-gray-200 ${compact ? 'pb-4' : 'pb-6'} ${featured ? 'md:col-span-2 md:grid md:grid-cols-[1.25fr_.75fr] md:gap-7' : ''}`}>
      ...
      <Link href={`/experiences/${e.id}`} className="contents">
        <div className={`relative overflow-hidden bg-gray-100 ${featured ? 'aspect-[16/10] md:aspect-auto md:min-h-[360px]' : compact ? 'aspect-square' : 'aspect-[4/3]'}`}>
          {e.images[0] ? (
        ...
        <div className={`flex flex-col ${compact ? 'pt-3' : 'pt-4'} ${featured ? 'md:justify-center md:pt-0' : ''}`}>
          ...
          <h3 className={`font-display font-bold leading-tight text-anthracite transition-colors group-hover:text-solar ${featured ? 'text-2xl md:text-4xl' : compact ? 'text-sm md:text-[15px]' : 'text-lg'}`}>{e.title}</h3>
          {!compact && <p className={`mt-3 text-gray-500 ${featured ? 'line-clamp-4 text-base leading-relaxed' : 'line-clamp-2 text-sm'}`}>{e.description}</p>}
          <div className={`${compact ? 'mt-3 pt-3 text-xs' : 'mt-5 pt-4 text-sm'} flex flex-wrap items-center justify-between gap-2 border-t border-gray-100`}>
            <div className="flex flex-wrap items-center gap-4 text-gray-500">
              <span className="flex items-center gap-1.5"><MapPin size={14} />{e.district || e.city}</span>
              {!compact && <span className="flex items-center gap-1.5"><Clock size={14} />{e.duration}</span>}
            </div>
